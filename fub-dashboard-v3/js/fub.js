 ...
 * fub.js
 * ─────────────────────────────────────────────
 * Follow Up Boss data layer.
 * All requests go through /api/fub (Vercel proxy) — never direct to FUB.
 * This avoids CORS and keeps auth headers server-side.
 */
...
const FUB = (() => {
  ...
  async function request(apiKey, path) {
    ...
    try {
      ...
      throw new Error("Non-JSON response from proxy (status " + resp.status + ")");
    }

    if (!resp.ok) {
      throw new Error(body.error || body.errorMessage || "FUB API error " + resp.status);
    }
  ...
  async function fetchAll(apiKey) {
    ...
    const people =
      peopleData.people ||
      (peopleData._embedded && peopleData._embedded.people) ||
      [];
    const events =
      eventsData.events ||
      (eventsData._embedded && eventsData._embedded.events) ||
      [];
  ...
  function buildFirstCallMap(callEvents) {
    ...
    callEvents.forEach((e) => {
      ...
      if (!pid) return;
      const t = new Date(
        e.created || e.createdAt || e.created_at
      ).getTime();
  ...
  function recentLeads(people, days) {
    ...
    return people.filter(
      (p) => new Date(p.created).getTime() > cutoff
    );
  }
  ...
  function speedToDialStats(leads, firstCallMap) {
    ...
    leads.forEach((p) => {
      ...
      const mins = Math.round(
        (fc - new Date(p.created).getTime()) / 60000
      );
      if (mins < 0) return;
      times.push(mins);
    ...
    return { avg, under5, under60, notCalled };
  }
  ...
  function _motLevel(p) {
    ...
    if (
      score >= CONFIG.HOT_SCORE ||
      tags.includes("hot") ||
      stage.includes("contract") ||
      stage.includes("active buyer") ||
      stage.includes("urgent")
    )
      return "hot";

    if (
      score >= CONFIG.WARM_SCORE ||
      tags.includes("warm") ||
      stage.includes("appt") ||
    ...
    if (
      tags.includes("nurture") ||
      stage.includes("nurture") ||
      stage.includes("long term") ||
      stage.includes("future")
    )
      return "nurture";
  ...
  function parseConcerns(people) {
    const cats = {
      "Not ready / timing": [
        "not ready", "timing", "not sure when",
        "future", "eventually", "someday", "too soon",
      ],
      "Price / budget": [
        "price", "afford", "budget",
        "expensive", "too much", "low budget", "cost",
      ],
      "Still renting / lease": [
        "rent", "lease", "landlord", "renting", "my lease",
      ],
      "Needs to sell first": [
        "sell first", "selling", "current home",
        "list my", "my house", "sell mine",
      ],
      "Just browsing": [
        "just looking", "browsing", "just checking",
        "not serious", "curious",
      ],
      "Has another agent": [
        "other agent", "already have", "working with",
        "my agent", "my realtor",
      ],
      "Credit / financing": [
        "credit", "financing", "pre-approval",
        "loan", "mortgage", "qualify", "approval",
      ],
      "Market concerns": [
        "market", "prices dropping", "wait for rates",
        "interest rate", "wait to buy",
      ],
    };
  ...
  function speedLabel(mins) {
    if (mins === null || mins === undefined)
      return { text: "No call yet", cls: "speed-slow" };
    if (mins < 0) return { text: "—", cls: "speed-slow" };
    if (mins <= CONFIG.SPEED_EXCELLENT)
      return { text: mins + "m", cls: "speed-fast" };
    if (mins <= CONFIG.SPEED_GOOD)
      return { text: mins + "m", cls: "speed-med" };
