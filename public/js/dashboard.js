const Dashboard = (() => {
  let rawData = null;      // full unfiltered data from server
  let filtered = null;     // currently filtered data
  let firstCallMap = {};
  let allLeads = [];
  ...
  // ── Poll server until data is ready ─────────────────────────────────────────
  async function waitForData() {
    ...
    pollInterval = setInterval(async () => {
      ...
      if (el) el.textContent = status.progress || '';
      if (status.hasCache || (!status.syncing && status.counts.people > 0)) {
  ...
  function applyFilters() {
    ...
    if (month) {
      ...
      const inMonth = d => { const t = new Date(d).getTime(); return t >= start && t < end; };
      people = people.filter(p => inMonth(p.created));
      calls  = calls.filter(c  => inMonth(c.created || c.createdAt || c.created_at));
      notes  = notes.filter(n  => inMonth(n.created || n.createdAt));
      appts  = appts.filter(a  => inMonth(a.created || a.createdAt || a.startTime));
    }
  ...
  function render(data) {
    ...
    if (agents.length) {
      ...
      set('ag-topDialerSub', topDialer.calls + ' calls');
  ...
  function renderSparklines(counts) {
    ...
    ['ov-spark1','ov-spark2','ov-spark3','ov-spark4'].forEach((id,ci) => {
      ...
      el.innerHTML = r.map((v,i) => {
        const h = Math.max(10,Math.round(v/max*100)), last = i===r.length-1;
        return `<div class="spark-bar ${last?'today':''}" style="height:${h}%;background:${last?cs[ci]+'1)':cs[ci]+'0.25)'}"></div>`;
      }).join('');
    });
  }
  ...
  function renderCallVolBars(vol) {
    ...
    el.innerHTML = vol.counts.map((v,i) => {
      const h = Math.max(4,Math.round(v/max*100)), last = i===vol.counts.length-1;
      return `<div style="flex:1;height:${h}%;background:${last?'var(--accent)':'rgba(0,229,160,0.2)'};border-radius:2px 2px 0 0" title="${vol.labels[i]}: ${v} calls"></div>`;
    }).join('');
  ...
  function renderFunnel(leads, contacted, appts) {
    ...
    const steps = [
      ['New leads', leads, '100%', 'var(--accent)', '100%'],
      ['Attempted contact', contacted, leads?Math.round(contacted/leads*100)+'%':'—', 'var(--accent)', '85%'],
      ['Connected', contacted, leads?Math.round(contacted/leads*100)+'%':'—', 'var(--accent4)', '65%'],
      ['Appointment set', appts, contacted?Math.round(appts/contacted*100)+'%':'—', 'var(--accent3)', '45%'],
    ];
    el.innerHTML = steps.map(([name,count,pct,color,w],i)=>`
      ${i>0?`<div class="funnel-drop">↓ ${steps[i][2]} advance</div>`:''}
  ...
  function renderHotLeads(people, elId) {
    ...
    el.innerHTML = hot.map(p => {
      ...
      const contactColor = days===null?'var(--danger)':days===0?'var(--accent)':days<=3?'var(--accent4)':'var(--danger)';
      return `<div style="padding:11px;background:var(--surface2);border-radius:8px;border:1px solid rgba(0,229,160,0.15);margin-bottom:7px">
        ...
        <div style="font-size:10px;color:var(--muted);line-height:1.7">Source: ${p.source||'—'}<br>Last contact: <span style="color:${contactColor}">${days===null?'Never':days===0?'Today':days+'d ago'}</span></div>
      </div>`;
    }).join('');
  }
  ...
  function renderCollectionCards(collections, total) {
    ...
    el.innerHTML = Object.entries(collections).filter(([,v])=>v>0).map(([k,v])=>`
      <div class="card card-accent-tl ${caMap[k]||''}">
        ...
        <div class="card-sub">${total?Math.round(v/total*100):0}% of filtered</div>
      </div>`).join('');
  }
  ...
  function filterLeads(q) {
    ...
    renderLeadsTable(people.filter(p=>[p.firstName,p.lastName,p.source,p.stage,p.lastNote].filter(Boolean).join(' ').toLowerCase().includes(query)));
  }
  ...
  function runAI() {
    ...
    const collections = FUB.collectionBreakdown(people);
    AI.getInsights({
      avgMins: speed.avg, under5: speed.under5, under60: speed.under60,
      notCalled: speed.notCalled, newLeads: people.length,
      totalPeople: rawData?.totalPeople || people.length,
