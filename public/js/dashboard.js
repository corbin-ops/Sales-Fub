const Dashboard = (() => {
  let rawData = null, filtered = null, firstCallMap = {}, allFirstCallMap = {};
  ...
  function buildMonthFilter() {
    ...
    if (!sel) return;
    const now = new Date(), cur = now.getMonth() + 1, yr = now.getFullYear();
    while (sel.options.length > 1) sel.remove(1);
    ['January','February','March','April','May','June','July','August','September','October','November','December']
      .slice(0, cur).forEach((name, i) => {
        const opt = document.createElement('option');
        opt.value = `${yr}-${String(i+1).padStart(2,'0')}`;
        opt.textContent = `${name} ${yr}`;
        sel.appendChild(opt);
      });
  }
  ...
  function populateAgentFilter(users) {
    ...
    users.forEach(u => {
      ...
      if ((u.name || '').toLowerCase().includes('marie')) marieUserId = u.id;
      sel.appendChild(opt);
    });
    sel.value = cur;
  }
  ...
  async function waitForData() {
    ...
    const poll = setInterval(async () => {
      const s = await fetch('/api/cache/status').then(r => r.json()).catch(() => null);
      if (!s) return;
      const el = document.getElementById('syncStatus');
      if (el) el.textContent = s.progress || '';
      if (s.hasCache) { clearInterval(poll); await loadDashboard(); }
    }, 3000);
  }
  ...
  async function loadDashboard() {
    ...
    try {
      ...
      rawData = raw;
      allFirstCallMap = FUB.buildFirstCallMap(raw.calls || []);
      buildMonthFilter();
  ...
  function applyFilters() {
    ...
    render(filtered);
    updateMarieDailyGoals();
  }

  // ── Marie goals — always TODAY, always unfiltered ──────────────────────────
  function updateMarieDailyGoals() {
    if (!rawData) return;
    const todayStart = new Date(new Date().toDateString()).getTime();
    const todayEnd   = todayStart + 86400000;
    const isToday = d => { const t = new Date(d).getTime(); return t >= todayStart && t < todayEnd; };
    const isMarie = x => marieUserId && (x.userId === marieUserId || x.user_id === marieUserId);

    const todayCalls = (rawData.calls || []).filter(c => isToday(c.created || c.createdAt || c.created_at) && isMarie(c));
    const dials = todayCalls.length;
    const talkMins = Math.round(todayCalls.reduce((s, c) => s + Number(c.duration || c.durationSeconds || 0), 0) / 60);
    const todayAppts = (rawData.appointments || []).filter(a => isToday(a.created || a.createdAt) && isMarie(a)).length;

    const todayLeads = (rawData.people || []).filter(p => isToday(p.created));
    const speeds = todayLeads.map(p => {
      const fc = allFirstCallMap[p.id];
      if (!fc) return null;
    ...
    renderGoal('goal-appts', 'goal-appts-bar', 'goal-appts-pct', todayAppts, GOALS.appts, 'var(--accent)', v => v + ' appts');
    renderGoal('goal-talk',  'goal-talk-bar',  'goal-talk-pct',  talkMins,   GOALS.talkMins, 'var(--accent3)', v => FUB.fmtMins(v));
    renderGoal('goal-dials', 'goal-dials-bar', 'goal-dials-pct', dials,      GOALS.dials, 'var(--accent4)', v => v + ' dials');

    const sEl = document.getElementById('goal-speed');
    const sBar = document.getElementById('goal-speed-bar');
    const sPct = document.getElementById('goal-speed-pct');
    if (sEl) sEl.textContent = avgSpeed !== null ? FUB.fmtMins(avgSpeed) : '—';
    if (sBar && avgSpeed !== null) {
      const good = avgSpeed <= GOALS.speedMins;
      sBar.style.width = Math.min(100, Math.round(GOALS.speedMins / Math.max(avgSpeed, 1) * 100)) + '%';
      sBar.style.background = good ? 'var(--accent)' : 'var(--danger)';
    }
    if (sPct) sPct.textContent = avgSpeed !== null ? (avgSpeed <= GOALS.speedMins ? '✓ Under 30m goal' : avgSpeed - GOALS.speedMins + 'm over goal') : 'no data today';
  }

  function renderGoal(valId, barId, pctId, actual, goal, color, fmt) {
    set(valId, marieUserId ? fmt(actual) : '—');
    const pct = Math.min(100, Math.round(actual / goal * 100));
    const met = actual >= goal;
    const bar = document.getElementById(barId);
    if (bar) { bar.style.width = (marieUserId ? pct : 0) + '%'; bar.style.background = met ? color : 'rgba(255,71,87,0.6)'; }
    const p = document.getElementById(pctId);
    if (p) { p.style.color = met ? color : 'var(--danger)'; p.textContent = marieUserId ? (met ? '✓ Goal met' : pct + '% of goal') : '—'; }
  }
  ...
  function render(data) {
    ...
    const { counts: collections, activeTotal } = FUB.collectionBreakdown(people);
    ...
    // OVERVIEW
    set('ov-newleads', people.length.toLocaleString());
    ...
    renderFunnel(people.length, contacted, apptCount, 'ovFunnel');
    ...
    set('sc-u5-pct', people.length ? Math.round(speed.under5 / people.length * 100) + '% of new leads' : '');
    renderFunnel(people.length, contacted, apptCount, 'scFunnel');
    renderHotLeads(people, 'scHotLeads');

    // SPEED TO FIRST CALL TAB
    const avgMins = speed.avg;
    set('sd-avg', avgMins !== null ? (avgMins >= 60 ? Math.floor(avgMins/60) + 'h' : avgMins + '') : '—');
    set('sd-unit', avgMins !== null && avgMins >= 60 ? (avgMins % 60 > 0 ? (avgMins % 60) + 'm' : '') : 'min');
    ...
    // PIPELINE
    renderCollectionCards(collections, activeTotal);
    ...
    // AI — pass raw unfiltered data so it can slice by month itself
    AI.getInsights({
      rawCalls: rawData ? rawData.calls : calls,
      rawPeople: rawData ? rawData.people : people,
      rawAppts: rawData ? rawData.appointments : appointments,
      rawNotes: rawData ? rawData.notes : notes,
      agents,
      concerns, callInsights,
      collections, topStages: stages.slice(0,6),
    });
  }
  ...
  function renderAgentSpeedTable(agents) {
    const el=document.getElementById('agentSpeedTable'); if(!el)return;
    const rows=agents.map(a=>`<tr>
      <td style="color:var(--text);font-weight:500">${a.name}</td>
      <td style="color:var(--muted);font-size:10px">${a.role||'—'}</td>
      <td style="color:var(--accent);font-family:'Syne',sans-serif;font-weight:700">${a.calls}</td>
      <td>${a.avgSpeed!==null?`<span class="spd ${speedCls(a.avgSpeed)}">${fmtSpeed(a.avgSpeed)}</span>`:'<span style="color:var(--muted)">—</span>'}</td>
      <td style="color:var(--muted)">${a.speeds?.length||0} leads timed</td>
    </tr>`).join('');
    el.innerHTML=`<table class="agent-table"><thead><tr><th>Agent</th><th>Role</th><th>Calls</th><th>Avg speed to first call</th><th>Sample size</th></tr></thead><tbody>${rows}</tbody></table>`;
  }

  function renderFunnel(leads, contacted, appts, elId) {
    const el=document.getElementById(elId); if(!el)return;
    const steps=[['New leads',leads,'100%','var(--accent)','100%'],['Contacted',contacted,leads?Math.round(contacted/leads*100)+'%':'—','var(--accent4)','75%'],['Appointment set',appts,contacted?Math.round(appts/contacted*100)+'%':'—','var(--accent3)','50%']];
  ...
  function renderAgentTable(agents) {
    ...
    const rows=agents.map(a=>`<tr>
      ...
      <td>${a.avgSpeed!==null?`<span class="spd ${speedCls(a.avgSpeed)}">${fmtSpeed(a.avgSpeed)}</span>`:'<span style="color:var(--muted)">—</span>'}</td>
      <td style="color:var(--muted)">${FUB.fmtMins(a.totalCallMins)}</td>
    </tr>`).join('');
    el.innerHTML=`<table class="agent-table"><thead><tr><th>Agent / Role</th><th>Dials</th><th>Connects</th><th>Connect %</th><th>Appts</th><th>Notes</th><th>Avg speed to first call</th><th>Talk time</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  ...
  function renderCollectionCards(collections, total) {
    ...
    el.innerHTML=activeEntries.map(([k,v])=>`<div class="card card-accent-tl ${caMap[k]||''}"><div class="card-label">${k}</div><div class="card-value" style="color:${colColors[k]||'var(--text)'}">${v.toLocaleString()}</div><div class="card-sub">${total?Math.round(v/total*100):0}% of active pipeline</div></div>`).join('')
      +`<div class="card" style="border-color:rgba(255,255,255,0.04)"><div class="card-label">Active pipeline total</div><div class="card-value" style="color:var(--text)">${total.toLocaleString()}</div><div class="card-sub">working stages only</div></div>`;
  }
  ...
  function runAI() {
    ...
    const stages=FUB.stageBreakdown(people);
    const sources=FUB.sourceBreakdown(people);
    const {counts:collections}=FUB.collectionBreakdown(people);
    AI.getInsights({
      rawCalls: rawData ? rawData.calls : calls,
      rawPeople: rawData ? rawData.people : people,
      rawAppts: rawData ? rawData.appointments : appointments,
