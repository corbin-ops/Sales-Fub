...
// ── Sync status checker — polls until full sync completes ─────────────────────
async function checkSyncStatus(apiKey) {
  try {
    const resp = await fetch("/api/cache/status", { headers: { "x-fub-key": apiKey } });
...
window.triggerFullSync = async function() {
  const apiKey = document.getElementById("apiKey")?.value.trim();
  if (!apiKey) { alert("Enter your FUB API key first"); return; }
  const resp = await fetch("/api/cache/full-sync", { method: "POST", headers: { "x-fub-key": apiKey } });
  const data = await resp.json();
  alert(data.message + "\n\nThis will take 5–15 minutes for 181K contacts. The dashboard will show live data while it syncs in the background.");
};

window.triggerIncrSync = async function() {
  const apiKey = document.getElementById("apiKey")?.value.trim();
  if (!apiKey) { alert("Enter your FUB API key first"); return; }
  const resp = await fetch("/api/cache/incr-sync", { method: "POST", headers: { "x-fub-key": apiKey } });
  const data = await resp.json();
